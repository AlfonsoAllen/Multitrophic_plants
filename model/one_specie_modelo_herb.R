...
#in this script i am going to use the HERBIVORY
#############
#######                     HERVIVORY
##########
...
herb.lema <- cov.list$LEMA["herb1"]
herb.lema <- list(herb.lema)
...
herb.lema <- data.frame(herb.lema)

#modelo
modelo.lema.junto.herb <- cxr_pm_fit(data = lema.junto,
                                    focal_column = sp,
                                    model_family = "BH",
                                    optimization_method = "GenSA", 
                                    covariates = herb.lema,
                                    alpha_form = "pairwise",
                                    ...
                                    # no standard errors
                                    bootstrap_samples = 0) 
summary(modelo.lema.junto.herb)
sp.intra.lema <- c("intra")
modelo.intrainter.herb <- cxr_pm_fit(data = lema.intrainter,
...
summary(modelo.intrainter.herb)
...
# discard the plant column  and herb and fv  
herb.chfu <- cov1.chfu$CHFU["herb1"]
...
#modelo
modelo.chfu.junto.herb <- cxr_pm_fit(data = chfu.junto,
                                    focal_column = sp1,
                                    model_family = "BH",
                                    optimization_method = "GenSA", 
                                    covariates = herb.chfu,
                                    alpha_form = "pairwise",
                                    lambda_cov_form = "global", # effect of covariates over lambda
                                    alpha_cov_form = "global", # effect of covariates over alpha
                                    initial_values = list(lambda = 1,
...
summary(modelo.chfu.junto.herb)
spi.intra <- c("intra")
modelo.chfu.intrainter.herb <- cxr_pm_fit(data = chfu.intrainter,
                                         ...
                                         # no standard errors
                                         bootstrap_samples = 0) 
summary(modelo.chfu.intrainter.herb)
...
# discard the plant column  and herb and fv  
herb.pupa <- cov1.pupa$PUPA["herb1"]
...
modelo.pupa.junto.herb <- cxr_pm_fit(data = pupa.junto,
                                    focal_column = sp1.pupa,
                                    model_family = "BH",
                                    optimization_method = "GenSA", 
                                    covariates = herb.pupa,
                                    alpha_form = "pairwise",
                                    lambda_cov_form = "global", # effect of covariates over lambda
                                    alpha_cov_form = "global", # effect of covariates over alpha
                                    initial_values = list(lambda = 1,
                                    ...
                                    # no standard errors
                                    bootstrap_samples = 0) 
summary(modelo.pupa.junto.herb)
sp1.pupa.intra <- c("intra")
modelo.pupa.intrainter.herb <- cxr_pm_fit(data = pupa.intrainter,
...
# load covariates: salinity
head(cov.list)
herbivory <- cov.list[my.sp]
# keep only salinity column
for(i in 1:length(herbivory)){
    herbivory[[i]] <- as.matrix(herbivory[[i]][,2])
    colnames(herbivory[[i]]) <- "herbivory"
}

modelo.fit_3sp_herb <- cxr_pm_multifit(data = obs_3sp,
                                  focal_column = my.sp,
                                  model_family = "BH",
                                  # here we use a bounded method for demonstration purposes
                                  optimization_method = "GenSA", 
                                  covariates = herbivory,
                                  alpha_form = "pairwise",
                                  lambda_cov_form = "global", # effect of covariates over lambda
                                  alpha_cov_form = "global", # effect of covariates over alpha
                                  initial_values = list(lambda = 1,
