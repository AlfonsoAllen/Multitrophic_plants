#que voy a #In this script i'm going to do the model to obtain lambda, alphas and the cov for the nexts models
#in this script i am going to use the FLORAL VISITORS
#############
#######                   FLORAL VISITORS
##########
...
herb.lema1 <- cov1.lema[sp]
# discard the plant column  and herb and fv  
pol.lema <- cov.list$LEMA["fv1"]
pol.lema <- list(pol.lema)
...
pol.lema <- data.frame(pol.lema)

#modelo
modelo.lema.junto.pol <- cxr_pm_fit(data = lema.junto,
                                         focal_column = sp,
                                         model_family = "BH",
                                         ...
                                         # no standard errors
                                         bootstrap_samples = 0) 
summary(modelo.lema.junto.pol)
sp.intra.lema <- c("intra")
modelo.intrainter.lema.pol <- cxr_pm_fit(data = lema.intrainter,
...
summary(modelo.intrainter.lema.pol)
...
#modelo
modelo.chfu.junto.pol <- cxr_pm_fit(data = chfu.junto,
                                         focal_column = sp1,
                                         model_family = "BH",
                                         optimization_method = "GenSA", 
                                         covariates = pol.chfu,
...
summary(modelo.chfu.junto.pol)
spi.intra <- c("intra")
modelo.chfu.intrainter.pol <- cxr_pm_fit(data = chfu.intrainter,
                                              ...
                                              # no standard errors
                                              bootstrap_samples = 0) 
summary(modelo.chfu.intrainter.pol)
...
# number of salinity observations
nrow(pol.pupa[1])
any(is.na(pupa.junto))

modelo.pupa.junto.pol <- cxr_pm_fit(data = pupa.junto,
                                         ...
                                         # no standard errors
                                         bootstrap_samples = 0) 
summary(modelo.pupa.junto.pol)
sp1.pupa.intra <- c("intra")
modelo.pupa.intrainter.pol <- cxr_pm_fit(data = pupa.intrainter,
...
summary(modelo.pupa.intrainter.pol)
...
# load covariates: salinity
head(cov.list)
f.visitors <- cov.list[my.sp]
# keep only salinity column
for(i in 1:length(f.visitors)){
    f.visitors[[i]] <- as.matrix(f.visitors[[i]][,3])
    colnames(f.visitors[[i]]) <- "f.visitors"
}

modelo.fit_3sp_pol <- cxr_pm_multifit(data = obs_3sp,
                                       focal_column = my.sp,
                                       model_family = "BH",
                                       optimization_method = "GenSA", 
                                       covariates = f.visitors,
                                       alpha_form = "pairwise",
                                       lambda_cov_form = "global", # effect of covariates over lambda
                                       alpha_cov_form = "global", # effect of covariates over alpha
                                       initial_values = list(lambda = 1,
